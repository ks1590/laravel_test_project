<div class="overflow-x-scroll m-8 w-full">
    <table class="w-full">
        <thead class="whitespace-nowrap bg-gray-50">
        <tr>
            ...
            <tr class="bg-white lg:hover:bg-gray-100">
                ...
                @foreach($shops as $shop)
                    <x-table.common.store-stock-body
                        :shop="$shop"
                        :filterItem="is_null($shop->items->find($item->sku)) ? '' : $shop->items->find($item->sku)->sku"
                        :itemSku="$item->sku"
                        :stocks="$stocks"
                ...
                @endforeach
                @if(auth()->user()->is_admin === 1)
                    <td class="py-3 border-b border-grey-light flex justify-center">
                        <div class="mr-3">
                            <x-edit-button
